...
def main():
    ...
    parser.add_argument(
        ...
        default="sae",
        choices=["sae", "token"],
        help="Probe basis: 'sae' (Gemma Scope decoders) or 'token' (tied embed/unembed matrix, routing validation only)"
    )
    parser.add_argument(
        "--sae-offset",
        type=int,
        default=None,
        help="Override sae_layer_offset_for_attn (default -1: attention layer L "
             "probed with the layer L-1 Gemma Scope SAE; 0 probes with layer L's own SAE)"
    )

    args = parser.parse_args()

    if args.sae_offset is not None:
        # The validator resolves SAE layers through the module-level singleton
        from config import GEMMA2_CONFIG
        GEMMA2_CONFIG.sae_layer_offset_for_attn = args.sae_offset
        print(f"SAE layer offset overridden: {args.sae_offset}")

    if args.basis == "token" and (args.validate_writing or args.writing_ablation or args.validate_ov):
        parser.error("--basis token supports routing validation only; "
                     "drop --validate-writing/--writing-ablation/--validate-ov "
                     "(OV reconstruction semantics are Gemma-Scope-specific)")
    ...
    if args.basis == "token":
        from token_basis import TokenBasisManager
        sae_manager = TokenBasisManager(
            model, tokenizer, gemma_config, subset_size=args.sae_features
        )
        print(f"Probe basis: token-embedding (subset={args.sae_features}, centered)")
    else:
